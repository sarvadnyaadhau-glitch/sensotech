...
import { sensorData } from "./FarmDashboard";
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
        ...
        {/* Live data bar */}
        <div className="flex items-center gap-3 mx-4 mt-4 rounded-xl px-4 py-3" style={{ background: "rgba(74, 222, 128, 0.08)", border: "1px solid rgba(74, 222, 128, 0.2)" }}>
          <div className="w-2 h-2 rounded-full bg-green-400 animate-pulse" />
          <span className="text-white/70 text-xs">Live sensor analysis</span>
          <div className="flex gap-2 ml-1">
            <span className="px-2 py-0.5 rounded-full text-xs" style={{ background: "rgba(96, 165, 250, 0.15)", color: "#60a5fa" }}>💧 {sensorData.moisture}%</span>
            <span className="px-2 py-0.5 rounded-full text-xs" style={{ background: "rgba(253, 224, 71, 0.15)", color: "#fde047" }}>pH {sensorData.ph}</span>
            <span className="px-2 py-0.5 rounded-full text-xs" style={{ background: "rgba(74, 222, 128, 0.15)", color: "#4ade80" }}>N {sensorData.nitrogen}</span>
          </div>
        </div>
        ...
        <div className="flex-1 px-4 pt-5 pb-10 overflow-y-auto">
          {sections.map((section) => (
            <div key={section.id} className="ai-section" style={{ background: section.bgColor, border: `1px solid ${section.borderColor}` }}>
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-2">
                  <div className="w-9 h-9 rounded-xl flex items-center justify-center text-lg" style={{ background: "rgba(0,0,0,0.3)" }}>{section.icon}</div>
                  <h3 className="text-white font-bold text-base">{section.title}</h3>
                </div>
                <span className="text-xs px-2 py-0.5 rounded-full font-bold flex-shrink-0" style={{ background: "rgba(0,0,0,0.3)", color: section.badgeColor, border: `1px solid ${section.badgeColor}40` }}>{section.badge}</span>
              </div>
              <p className="text-white/85 text-sm leading-relaxed mb-3">{section.content}</p>
              <div className="rounded-xl p-3 mb-3" style={{ background: "rgba(0,0,0,0.3)", border: "1px solid rgba(255,255,255,0.06)" }}>
          ...
          {/* Overall assessment */}
          <div className="rounded-2xl p-5 mt-2" style={{ background: "rgba(74, 222, 128, 0.08)", border: "1px solid rgba(74, 222, 128, 0.3)" }}>
            ...
            <p className="text-white/75 text-sm leading-relaxed mb-4">
              {lang === "mr"
                ? <><span className="text-green-400 font-bold">माउली शेत चांगले कामगिरी करत आहे.</span> तुमची लिंबाची बाग निरोगी आहे. मुख्य आवश्यक कृती म्हणजे नायट्रोजन पूरक. वरील खत मार्गदर्शकाचे पालन करा आणि पुढील <span className="text-yellow-300 font-bold">8-10 आठवड्यांत</span> अंदाजे <span className="text-green-300 font-bold">~1,200 kg</span> उत्पन्न अपेक्षित आहे.</>
                : lang === "hi"
                ? <><span className="text-green-400 font-bold">माउली खेत अच्छा प्रदर्शन कर रहा है।</span> आपका नींबू का बाग स्वस्थ है। मुख्य कार्रवाई नाइट्रोजन पूरकता है। ऊपर के उर्वरक गाइड का पालन करें और अगली फसल <span className="text-yellow-300 font-bold">8-10 हफ्तों</span> में <span className="text-green-300 font-bold">~1,200 kg</span> उपज की उम्मीद है।</>
                : <><span className="text-green-400 font-bold">Mauli Farm is performing well.</span> Your lemon orchard is in good health. The primary action needed is nitrogen supplementation. Your next estimated harvest will be in <span className="text-yellow-300 font-bold">8-10 weeks</span> with an expected yield of <span className="text-green-300 font-bold">~1,200 kg</span>.</>
              }
            </p>
            <div className="flex items-center gap-3 rounded-xl p-3" style={{ background: "rgba(0,0,0,0.3)" }}>
              ...
              <div className="text-center flex-1">
                ...
                <p className="text-white/50 text-xs">{lang === "mr" ? "कृती" : lang === "hi" ? "कार्रवाई" : "Actions"}</p>
              </div>
              <div className="w-px h-8" style={{ background: "rgba(74, 222, 128, 0.2)" }} />
              <div className="text-center flex-1">
                <p className="text-blue-300 font-black text-xl">8w</p>
                <p className="text-white/50 text-xs">{lang === "mr" ? "कापणीपर्यंत" : lang === "hi" ? "फसल तक" : "To Harvest"}</p>
              </div>
            </div>
          </div>
        </div>
